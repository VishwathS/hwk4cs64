...
doSwap:
        ...
        la $t3, myArray      # t3 = address of myArray[0]
        sll $t4, $t0, 2      # t4 = shift (x * 4)
        add $t5, $t4, $t3
        lw $t6, 0($t5)       # t6 = temp

        sll $t7, $t1, 2
        add $t4, $t3, $t7   # t4 = address of myArray[y]
        lw $t7, 0($t4)      # t7 = myArray[y]

        sw $t7, 0($t5)      # myArray[x] = myArray[y]
        sw $t6, 0($t4)      # myArray[y] = temp
